...
with st.sidebar:
    ...
    # --- BLOCO 2: HISTÓRICOS DE CONVERSAS DE PDFS E LIVROS ---
    ...
    # --- BLOCO 3: SELEÇÃO DE LIVROS E EXCLUSÃO ---
    ...
    # --- BLOCO 4: IMPORTAÇÃO E PROCESSAMENTO DE PDFs ---
    with st.expander("📥 Importar Novos PDFs", expanded=False):
        ...
        if arquivos_pdf and st.button("🚀 Processar e Vetorizar", use_container_width=True):
            ...
            st.rerun()
# --- BLOCO 5: HISTÓRICO DE CONVERSAS OBSIDIAN ---
    ...
    # --- BLOCO 6: PASTAS IGNORADAS (AGORA NO EXPANDER) ---
...
aba_chat_livros, aba_chat_obsidian, aba_linker = st.tabs(["📚 Chat Livros (PDF)", "📝 Chat Obsidian", "🔗 Gestor de Conexões"])


# ------------------------------------------
# ABA 1: CHAT LIVROS PDF (O SEU NOVO NOTEBOOKLM)
# ------------------------------------------
            ...
            with st.chat_message("assistant"):
                ...
                resposta_completa = st.write_stream(
                    engine_livros.query(
                        ...
                        top_k=top_k_livros,
                        modo_estrito=not conhecimento_externo_livro,
